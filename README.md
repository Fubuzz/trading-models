...
- The baseline feature set now also includes short/medium, short/long, and medium/long moving-average spread features (`ma_10_vs_ma20`, `ma_10_vs_ma50`, `ma_20_vs_ma50`) so the model can distinguish trend alignment across multiple horizons from simple price-vs-average distance without reducing the usable sample history.
- The feature set now also includes `ma_stack_bullish_count`, a discrete count of bullish moving-average alignments across the 10/20/50-day stack (`ma_10 > ma_20`, `ma_10 > ma_50`, `ma_20 > ma_50`); this gives the classifier a compact trend-breadth summary while reusing the existing 50-day moving-average warmup.
- The feature set now also includes `ma_10_slope_5d`, defined as the 5-day change in the 10-day moving average; this gives the classifier a short-trend slope signal without increasing warmup beyond the existing moving-average stack, and zero-denominator windows are treated as a neutral `0.0`.
...
- The feature set now also includes `rebound_from_low_20`, defined as the percentage lift from the rolling 20-day low; this complements the drawdown feature with an explicit rebound-strength signal while reusing the same 20-day range window, and zero rolling lows are treated as a neutral `0.0` for robustness.
- The feature set now also includes `range_width_pct_20`, defined as `range_high_20 / range_low_20 - 1`; this gives the classifier an explicit normalized 20-day trading-range width signal while reusing the existing high/low window, and zero rolling lows are treated as a neutral `0.0` for robustness.
...
- The feature set now also includes `rsi_14_change_5d`, defined as the 5-day change in the 14-period RSI; this gives the classifier an explicit oscillator-momentum signal that can distinguish strengthening versus fading overbought/oversold conditions without increasing warmup beyond the existing RSI stack, and flat-price windows naturally read as a neutral `0.0`.
